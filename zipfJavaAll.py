...
def openPickles():
	try:
	    zipfDic = pickle.load(open("allJava.p", "rb"))
	except (OSError, IOError) as e:
	    zipfDic = {}

	try:
		filesChecked = pickle.load(open("filesCheckedAJ.p", "rb"))
	except (OSError, IOError) as e:
		filesChecked = {}

	try:
		totalCount = pickle.load(open("totalCountAJ.p", "rb"))
	except (OSError, IOError) as e:
		totalCount = 0
...
def savePickle():
	pickle.dump(zipfDic, open("allJava.p", "wb"))
	pickle.dump(filesChecked, open("filesCheckedAJ.p", "wb"))
	pickle.dump(totalCount, open("totalCountAJ.p", "wb"))
