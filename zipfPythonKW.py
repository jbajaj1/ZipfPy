...
def openPickles():
	try:
	    zipfDic = pickle.load(open("kwPython.p", "rb"))
	except (OSError, IOError) as e:
	    zipfDic = {}

	try:
		filesChecked = pickle.load(open("filesCheckedKW.p", "rb"))
	except (OSError, IOError) as e:
		filesChecked = {}

	try:
		totalCount = pickle.load(open("totalCountKW.p", "rb"))
	except (OSError, IOError) as e:
		totalCount = 0
...
def savePickle():
	pickle.dump(zipfDic, open("kwPython.p", "wb"))
	pickle.dump(filesChecked, open("filesCheckedKW.p", "wb"))
	pickle.dump(totalCount, open("totalCountKW.p", "wb"))
...
def zipfCounterPythonKW(zipfDic, filesChecked, totalCount):
...
zipfDic, filesChecked, totalCount = zipfCounterPythonKW(zipfDic, filesChecked, totalCount)
